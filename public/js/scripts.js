...
document.addEventListener('DOMContentLoaded', function () {
    ...
    form.addEventListener('submit', function (e) {
        e.preventDefault();
        if (input.value) {
            // Emitting message with user's name (assuming the user's name is available on the client-side)
            socket.emit('chat message', { name: "User's Name", message: input.value });
            input.value = '';
        }
    });
});
...
socket.on('chat message', function (data) {
    const messages = document.getElementById('messages'); // Assuming you have an element to display messages
